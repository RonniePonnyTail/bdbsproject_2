...
if (isset($_POST['submitSong'])) {
	...
	if (empty($Kapela) || empty($Album) || empty($Pisen) || empty($Delka)) {
		...
		exit();
			...
			$sql = "SELECT * FROM hudba WHERE PISEN = '$Pisen' AND KAPELA = '$Kapela'";
			$result = mysqli_query($conn, $sql);
			$resultCheck = mysqli_num_rows($result);
			if ($resultCheck > 0) {
				header("Location: ../databaze.php?addSong=SongAlredyExist");
				exit();
			} else{
				if ($UserID == 0) {
					header("Location: ../databaze.php?addSong=EPICFAIL");
					exit();
				}
				else{
					//Vlozeni do databaze
					...
					header("Location: ../databaze.php?addSong=succes");
					exit();
				}
			}
		}
	...
	header("Location: ../databaze.php?addSong=ready");
	exit();
}